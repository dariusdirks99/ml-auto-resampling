...
def train_step(
    ...
    Args:
        ...
        sample_weights: Sample weights from resampling technique, or None.
    ...
    for batch, (x, y) in enumerate(dataloader):
        ...
        if sample_weights is not None:
            start = batch * x.size(0)
            end = start + x.size(0)
            batch_weights = torch.tensor(sample_weights[start:end]).to(device)
            loss = F.cross_entropy(y_pred, y, reduction="none")
            loss = torch.mean(loss * batch_weights)
        else:
